...
def read_list(usr, pwd, db):
    try:
        ...
        cursor = conn1.cursor()
        cursor.execute( 'SELECT * FROM restaurants where comm_num >50 order by  res_id limit 0,10')
...
for item in a :
    ...
    try:
        ...
        logger.error(e)
        error_resIdLog = open('error_resId.txt', 'a')
        error_resIdLog.writelines("时间："+ datetime.datetime.now().strftime("%Y-%m-%d %H:%M:%S")+"做标记失败的店铺："+region_id+"   "+region_link+"\n")
    set_flag(region_id)
    time.sleep(2)



# find_comment_onePage("http://www.dianping.com/shop/3259888/review_more")
# for item in a :
#     region_id=item[0] #店铺id 店铺名称 店铺链接
#     region_name=item[1]
#     region_link =item[2]
